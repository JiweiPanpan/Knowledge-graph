...
def ranking(head, relation, tail, head_t_w, relation_t_w, tail_t_w, ent_num, edge_index, kge_model):
    """
    ...
    all_entity = torch.LongTensor(np.arange(0, ent_num))
    head_expand = torch.ones(ent_num)
    tail_expand = torch.ones(ent_num)
    relation_expand = torch.ones(ent_num)
    ...
    with torch.no_grad():
        for idx in range(len(head)):
            ...
            h, r, t = h.type(torch.LongTensor), r.type(torch.LongTensor), t.type(torch.LongTensor)
            # expand word
            h_w, r_w, t_w = [head_t_w[idx]] * ent_num, [relation_t_w[idx]] * ent_num, [tail_t_w[idx]] * ent_num

            Corrupted_score_tail = kge_model.score(h, r, all_entity, h_w, r_w, t_w, edge_index,'training')
        ...
        # ranking_head = (argsort_head == h).nonzero(as_tuple=True)[0]
            ...
            #reciprocal_rank = 1 / (ranking_tail +ranking_head + 1)
            reciprocal_rank = 1 / (ranking_tail  + 1)
