...
def edge_index(dataset_train):
    ...
    # 收集实体和关系
    for triple in dataset_train:
        head_w, relation_w, tail_w = triple
        heads.add(head_w)
        tails.add(tail_w)
        relations.add(relation_w)
    ...
    for triple in dataset_train:
        ...
        edge_index_train.append([head_idx, tail_idx])

    # edge_index (tensor)
    edge_index_train = torch.tensor(edge_index_train, dtype=torch.long).t().contiguous()

    num_nodes_train = len(dataset_train)
    return edge_index_train, num_nodes_train